...
async def main():
    ...
    try:
        ...
        if registry.async_get(entity('mon')).disabled_by is not None:
        ...
        if '--reload' in sys.argv:
            pending = asyncio.create_task(hass.services.async_call('text', 'set_value',
                {'entity_id': entity('name'), 'value': 'HA reload test'}, blocking=True))
            try:
                for _ in range(100):
                    if coordinator.controller._lock.locked():
                        break
                    await asyncio.sleep(0.05)
                assert coordinator.controller._lock.locked(), 'Write did not acquire session lock'
                log('Reloading while name write holds BLE session lock')
                assert await hass.config_entries.async_reload(entry.entry_id)
                await pending
                await hass.async_block_till_done()
                assert entry.state.value == 'loaded'
                assert entry.runtime_data.controller._lock is coordinator.controller._lock
                assert entry.runtime_data.data.programs[1].name == 'HA reload test'
                log('PASS concurrent write and reload; new coordinator read verified name')
            finally:
                await asyncio.gather(pending, return_exceptions=True)
                await entry.runtime_data.async_set_program('B', name=baseline.programs[1].name)
                final = await entry.runtime_data.controller.read_data()
                assert final.programs == baseline.programs
                save('final_programs', [asdict(p) for p in final.programs])
                log('PASS original programs restored after concurrent reload')
            return
